...
import 'package:sqljocky5/internal/result_parser/binary_data_packet.dart';
...
import 'package:sqljocky5/public/results/row.dart';
...
class ExecuteQueryHandler extends HandlerWithResult {
  ...
  Uint8List writeValuesToBuffer(
    ...
    var buffer =
        FixedWriteBuffer(10 + nullMap.length + 1 + types.length + length);
  ...
  HandlerResponse processResponse(ReadBuffer response) {
    ...
    if (packet == null) {
      ...
      if ((packet.serverStatus & SERVER_MORE_RESULTS_EXISTS) == 0) {
        var stream =
            StreamedResults(_okPacket.insertId, _okPacket.affectedRows, null);
